...
export class DialogOcrComponent extends MzBaseModal implements OnInit {
  ...
  constructor(private api: KrameriusApiService, private shareService: ShareService, private settings: AppSettings, private translator: Translator) {
    super();
  }
...
private dataPush(mods) {
    for (const doctype of SolrService.allDoctypes) {
      if (this.metadata.modsMap[doctype]) {
        this.data.push({
          ...
          citation: this.generateCitation(this.metadata, this.metadata.modsMap[doctype].uuid, Number(this.doctypeLevel(doctype)), mods),
    ...
    if (this.metadata.activePage) {
      this.data.push({
        type: 'page',
        level: DialogOcrComponent.LEVEL_PAGE,
        citation: this.generateCitation(this.metadata, this.metadata.activePage.uuid, DialogOcrComponent.LEVEL_PAGE, mods),
        uuid: this.metadata.activePage.uuid
      });
    }
    this.data.reverse();
    if (this.data.length > 0) {
      this.citation=this.data[0]['citation'];
    }
  }

 private doctypeLevel(doctype): number {
    if(doctype=="page") {return DialogOcrComponent.LEVEL_PAGE;}
    else if(doctype=="periodicalvolume") {return DialogOcrComponent.LEVEL_VOLUME;}
    else if(doctype=="periodicalitem") {return DialogOcrComponent.LEVEL_ISSUE;}
    else if(doctype=="article") {return DialogOcrComponent.LEVEL_ARTICLE;}
    else { return DialogOcrComponent.LEVEL_DOCUMENT; }
 ...
 public generateCitation(metadata: Metadata, uuid: string, level: number = DialogOcrComponent.LEVEL_DOCUMENT, mods): string {
   if (!metadata) {
     return null;
   }
   ...
   if (metadata.article && level >= DialogOcrComponent.LEVEL_ARTICLE) {
   ...
   if (metadata.publishers.length > 0 && (level === DialogOcrComponent.LEVEL_DOCUMENT || level === DialogOcrComponent.LEVEL_PAGE && !metadata.volume)) {
   ...
   if (metadata.volume && level >= DialogOcrComponent.LEVEL_VOLUME) {
     if (metadata.publishers.length > 0) {
       c += metadata.publishers[0].placeAndName();
       c += ', ';
     }
     if (metadata.currentIssue && metadata.currentIssue.title && level >= DialogOcrComponent.LEVEL_ISSUE) {
     ...
     if (metadata.currentIssue && level >= DialogOcrComponent.LEVEL_ISSUE) {
     ...
     if (metadata.article && metadata.article.metadata && metadata.article.metadata.extent && level === DialogOcrComponent.LEVEL_ARTICLE) {
   ...
   if(mods) { var partNo=this.getParamFromXml('mods:partNumber', mods); if(partNo) { c += 'sv. '+partNo+' '; } }
   if (metadata.activePages && level === DialogOcrComponent.LEVEL_PAGE) {
