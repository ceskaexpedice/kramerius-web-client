...
private dataPush(mods) {
 ...
 public generateCitation(metadata: Metadata, uuid: string, level: number = DialogOcrComponent.LEVEL_DOCUMENT, mods): string {
   ...
   if (metadata.volume && level >= DialogOcrComponent.LEVEL_VOLUME) {
     ...
     if (metadata.currentIssue && metadata.currentIssue.date && level >= DialogOcrComponent.LEVEL_ISSUE) {
   ...
   if(metadata.currentIssue && level < DialogOcrComponent.LEVEL_ISSUE && mods) { var partNo=this.getParamFromXml('mods:partNumber', mods); if(partNo) { c += 'sv. '+partNo+' '; } }
